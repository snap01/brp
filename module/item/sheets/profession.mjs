...
export class BRPProfessionSheet extends BRPItemSheetV2 {
  ...
  static DEFAULT_OPTIONS = {
    ...
    form: {
      handler: BRPProfessionSheet.myProfessionHandler
    }
  }
  ...
  static PARTS = {
    ...
    skills: {
      template: 'systems/brp/templates/item/profession.skills.hbs',
      scrollable: ['']
    },
    powers: {
      template: 'systems/brp/templates/item/profession.powers.hbs',
      scrollable: ['']
    },
    wealth: { template: 'systems/brp/templates/item/profession.wealth.hbs' },
  ...
  async _prepareContext(options) {
    ...
    context.perPower = perPower.sort(BRPUtilities.sortByNameKey);
  ...
  _getTabs(parts) {
    ...
    if (!this.tabGroups[tabGroup]) {
      if (game.settings.get('brp', 'defaultTab')) {
        ...
        this.tabGroups[tabGroup] = 'skills';
      }
    }
  ...
  _configureRenderOptions(options) {
    ...
    options.parts = ['header', 'tabs', 'skills', 'powers', 'wealth', 'description'];
    if (game.user.isGM) {
  ...
  _onRender(context, options) {
    ...
    this.element.querySelectorAll('.item-power-delete').forEach(n => n.addEventListener("click", this._onPowersDelete.bind(this)));
  ...
  //--------------------HANDLER----------------------------------
  static async myProfessionHandler(event, form, formData) {
